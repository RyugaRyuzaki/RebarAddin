...
namespace R01_ColumnsRebar
{
    public class StirrupModel : BaseViewModel
    {
        ...
        private bool _IsTypeH;
        public bool IsTypeH { get => _IsTypeH; set { _IsTypeH = value; OnPropertyChanged(); } }
        private int _TypeH;
        public int TypeH { get => _TypeH; set { _TypeH = value; OnPropertyChanged(); } }
        private int _nH;
        public int nH { get => _nH; set { _nH = value; OnPropertyChanged(); } }
        private double _aH;
        public double aH { get => _aH; set { _aH = value; OnPropertyChanged(); } }

        private RebarBarModel _BarV;
        public RebarBarModel BarV { get => _BarV; set { _BarV = value; OnPropertyChanged(); } }
        private bool _AddV;
        public bool AddV { get => _AddV; set { _AddV = value; OnPropertyChanged(); } }
        private bool _IsTypeV;
        public bool IsTypeV { get => _IsTypeV; set { _IsTypeV = value; OnPropertyChanged(); } }
        private int _TypeV;
        public int TypeV { get => _TypeV; set { _TypeV = value; OnPropertyChanged(); } }
        private int _nV;
        public int nV { get => _nV; set { _nV = value; OnPropertyChanged(); } }
        private double _aV;
        ...
        #endregion
        public StirrupModel(int numberColumn, RebarBarModel barS, RebarBarModel barH, RebarBarModel barV, bool addH, bool addV, int typeDis, double s, double s1, double s2, bool isties, double ah, double av)
        {
            NumberColumn = numberColumn;
            BarS = barS; BarH = barH; BarV = barV;
            AddH = addH; AddV = addV; TypeH = 0; TypeV = 0; nH = 1; nV = 1; IsTypeH = AddH && TypeH == 0; IsTypeV = AddV && TypeV == 0; aH = ah; aV = av;
        ...
        private List<XYZ> GetOriginVectorStirrup(SectionStyle sectionStyle, InfoModel infoModel, UnitProject unit, double Cover, double location0)
        {
            ...
            else
            {
                ...
                double location = unit.Convert(location0);
                XYZ p2 = p1 + (D * 0.5 - cover) * (-1) * XYZ.BasisX;
                XYZ p3 = p2 + (D * 0.5 - cover) * (-1) * XYZ.BasisY;
        ...
        private List<XYZ> GetScaleBoxStirrup(SectionStyle sectionStyle, InfoModel infoModel, UnitProject unit, double Cover)
        {
            ...
            else
            {
                ...
                double D = unit.Convert(infoModel.D);
                XYZ p2 = p1 + (D * 0.5 - cover) * (-1) * XYZ.BasisX;
                XYZ p3 = p1 + (D * 0.5 - cover) * (-1) * XYZ.BasisY;
                XYZ p4 = p3 + (D - 2 * cover) * XYZ.BasisX;
                XYZ p4a = p3 + (D - 2 * cover) * XYZ.BasisY;
                XYZ pb = p4 - p3;
                vector.Add(pb);
                XYZ ph = p4a - p3;
                vector.Add(ph);
        ...
        private List<XYZ> GetOriginVectorAddStirrupCylindrical(InfoModel infoModel, UnitProject unit, double Cover, double location0)
        {
            List<XYZ> vector = new List<XYZ>();
            XYZ p1 = PointModel.ProjectToPlane(infoModel.PointPosition, infoModel.Bottom);
            double Dsqrt = unit.Convert(Math.Sqrt(2) * infoModel.D * 0.5 - Cover + BarH.Diameter);
            double location = unit.Convert(location0);
        ...
        private List<XYZ> GetOriginVectorAntiHorizontalCylindrical(InfoModel infoModel, UnitProject unit, double Cover, double location0)
        {
            ...
            XYZ p1 = PointModel.ProjectToPlane(infoModel.PointPosition, infoModel.Bottom);
            double cover = unit.Convert(Cover);
            double x = unit.Convert(infoModel.D / 2 )-cover;
            double location = unit.Convert(location0);
        ...
        private List<XYZ> GetScaleBoxAntiHorizontalCylindrical(InfoModel infoModel, UnitProject unit, double Cover)
        {
            ...
            double cover = unit.Convert(Cover);
            double x = unit.Convert(infoModel.D / 2 ) - cover;
            double h = unit.Convert(infoModel.D);
            XYZ p2 = p1 + x * (-1) * XYZ.BasisX;
            XYZ p3 = p2 + (unit.Convert(BarV.Diameter)) * (-1) * XYZ.BasisY;
        ...
        private List<XYZ> GetOriginVectorAntiVerticalCylindrical(InfoModel infoModel, UnitProject unit, double Cover, double location0)
        {
            ...
            XYZ p1 = PointModel.ProjectToPlane(infoModel.PointPosition, infoModel.Bottom);
            double cover = unit.Convert(Cover);
            double y = unit.Convert(infoModel.D / 2 ) - cover;
            double location = unit.Convert(location0);
        ...
        private List<XYZ> GetScaleBoxAntiVerticalCylindrical(InfoModel infoModel, UnitProject unit, double Cover)
        {
            ...
            double cover = unit.Convert(Cover);
            double y = unit.Convert(infoModel.D / 2 ) - cover;
            double b = unit.Convert(infoModel.D);
            XYZ p2 = p1 + y * XYZ.BasisY;
            XYZ p3 = p2 + (unit.Convert(BarV.Diameter)) * (-1) * XYZ.BasisX;
