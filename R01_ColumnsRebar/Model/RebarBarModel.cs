...
namespace R01_ColumnsRebar
{
    public class RebarBarModel : BaseViewModel
    {
        ...
        private List<Rebar> _RebarStirrup;
        public List<Rebar> RebarStirrup { get => _RebarStirrup; set { _RebarStirrup = value; OnPropertyChanged(); } }
        ...
        public void GetRebarShape(SectionStyle sectionStyle, Document document, string type, SettingModel settingModel)
        {

            if (type.Equals("Stirrup"))
            {
                //RebarShape = settingModel.RebarShapes.Where(x => x.Name == "M_T1").FirstOrDefault();
                if (sectionStyle==SectionStyle.RECTANGLE)
                {
                    RebarShape = settingModel.SelectedShapeStirrup;
                }
                else
                {
                   RebarShape = settingModel.RebarShapes.Where(x => x.Name == "M_T3").FirstOrDefault();
                }
               
            }
            if (type.Equals("Anti"))
            {
                //double hookAngle = settingModel.SelectedHook.get_Parameter(BuiltInParameter.REBAR_HOOK_ANGLE).AsDouble();
                //if (PointModel.AreEqual(hookAngle, Math.PI))
                //{
                //    RebarShape = settingModel.RebarShapes.Where(x => x.Name == "M_T10C").FirstOrDefault();
                //}
                //if (PointModel.AreEqual(hookAngle, 0))
                //{
                //    RebarShape = settingModel.RebarShapes.Where(x => x.Name == "M_T10A").FirstOrDefault();
                //}
                //if (PointModel.AreEqual(hookAngle, Math.PI / 2))
                //{
                //    RebarShape = settingModel.RebarShapes.Where(x => x.Name == "M_T10B").FirstOrDefault();
                //}
                //if (PointModel.AreEqual(hookAngle, 3 * Math.PI / 4))
                //{
                //    RebarShape = settingModel.RebarShapes.Where(x => x.Name == "M_T10").FirstOrDefault();
                //}
                RebarShape = settingModel.SelectedShapeAnti;
            }
            if (type.Equals("Stirrup Cylindrical"))
            {
                RebarShape = settingModel.RebarShapes.Where(x => x.Name == "M_T1").FirstOrDefault();
            }
        }
        ...
        public void SetPartitionRebarStirrup(string name)
        {
            for (int i = 0; i < RebarStirrup.Count; i++)
            {
                RebarStirrup[i].LookupParameter("Partition").Set(name);
            }
        }
        #region Tag
