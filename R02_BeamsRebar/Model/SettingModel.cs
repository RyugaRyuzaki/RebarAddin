...
namespace R02_BeamsRebar
{
    public class SettingModel:BaseViewModel
    {
        ...
        private double _L4;
        public double L4 { get => _L4; set { _L4 = value; OnPropertyChanged(); } }
        private string _BeamsName;
        public string BeamsName { get => _BeamsName; set { _BeamsName = value; OnPropertyChanged(); } }
        ...
        public SettingModel(double tmin, double dmin, double dimH, double dimV, double tagH, double tagV, double l1, double l2, double l3, double l4, string beamsName, string prefixSection, Element beam, Document document)
        {
            this.tmin = tmin;
            this.dmin = dmin;
            DimH = dimH;
            ...
            List<Parameter> a = beam.GetOrderedParameters().ToList();
            Parameters = new List<Parameter>();
            foreach (var item in a)
            {
                if (item.StorageType == StorageType.String && (!(item.Definition.Name == "Work Plane")))
                {
                    Parameters.Add(item);
                }
            }
            ...
            PrefixDetail = beam.get_Parameter(BuiltInParameter.INSTANCE_REFERENCE_LEVEL_PARAM).AsValueString();
            AllDimensionType = new FilteredElementCollector(document).OfClass(typeof(DimensionType)).Cast<DimensionType>().ToList();
            //SelectedDimensionType = AllDimensionType.Where(x => x.Name.Equals(TagName)).FirstOrDefault();
            SelectedDimensionType = AllDimensionType[0];
            GetDetailViewName();
            GetSectionViewName();
            ...
            if (AllDetailItemTag != null && AllDetailItemTag.Count != 0)
            {
        ...
        public void GetDetailViewName()
        {
            if (IsPrefixDetail)
            {
                DetailViewName = BeamsName + "-" + PrefixDetail;
            }
            else
            {
                DetailViewName = BeamsName;
            }
        }
