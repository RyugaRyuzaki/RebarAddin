...
namespace R02_BeamsRebar
{
    public class MainBottomBarModel : BaseViewModel
    {
        ...
        public MainBottomBarModel(int barNumber, int numberBar, RebarBarModel bar, double length, double la, double exa, double lb, double exb, double x, double y,double temp)
        {
            BarNumber = barNumber;
            NumberBar = numberBar;
            Bar = bar;
        ...
        public void CreateMainBottomBar(Document document, PlanarFace planarFace, UnitProject unit, double c, double dsmax, InfoModel infoModel, SettingModel settingModel)
        {
        ...
        #endregion
        #region Tag
        public void CreateTagRebarDetailMainBottom(Autodesk.Revit.DB.View view, Document document, UnitProject unit, List<InfoModel> infoModels, SettingModel settingModel, PlanarFace planarFace0, double h0, double v0)
        {
            for (int i = 0; i < infoModels.Count; i++)
            {
                ...
                {
                    Bar.CreateTagRebarDetailBottom(view, document, unit, infoModels[0], settingModel, planarFace0, x0, Location[1].Y, h0, v0);
                }
            }
        }
        public void CreateTagRebarSection(List<SectionBeamView> SectionBeamViews, Document document, UnitProject unit, List<InfoModel> infoModels, PlanarFace planarFace0, SettingModel settingModel, double tagH0, double tagV0)
        {
            for (int i = 0; i < infoModels.Count; i++)
            {
                ...
                if (Location[Location.Count - 1].X > x0 && Location[0].X < x0)
                {
                    Bar.CreateTagRebarSectionDown(SectionBeamViews[i].StartView, document, unit, infoModels[i], planarFace0, settingModel, Location[1].Y, tagH0, tagV0);
                    Bar.CreateTagRebarSectionDown(SectionBeamViews[i].MidView, document, unit, infoModels[i], planarFace0, settingModel, Location[1].Y, tagH0, tagV0);
                    Bar.CreateTagRebarSectionDown(SectionBeamViews[i].EndView, document, unit, infoModels[i], planarFace0, settingModel, Location[1].Y, tagH0, tagV0);
                }
            }
        }
        #endregion
